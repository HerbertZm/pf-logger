...
| `matches-list` paginates at 200, not capped | Large rounds have 700–1300 records (incl. byes at `table_number = -1`/`0`). Page through `next` for the true last result — a single page silently truncates. Dedupe by `table_number` (latest per table). |

...
| `timer_end_datetime` NOT recoverable for completed rounds | Round objects (`get_all_rounds`, `tournament_overview`, `detail.tournament_phases[].rounds`) carry only `started_at`, `timer_duration_minutes`, `completed_at`. The event `detail/` endpoint keeps only ONE event-level `timer_end_datetime` = the last timer state at event close. No round-detail / timer-history / audit-log endpoint exists (all 404). The worker must capture `timer_end_datetime` **live per round** — it cannot be backfilled afterward. For the round-timing report, scheduled end = real `timer_end_datetime`, round start = scheduled_end − 60; never `started_at + duration`. |
| `result_reported_at` absent from `matches-list` | This endpoint returns only `created_at` and `updated_at`. Use `updated_at` for result time (bumped by later edits). For true result-entry time, use `history_date` from `matches/{id}/history/`. |
| `matches-list` paginates at 200, not capped | Large rounds have 700–1300 records (incl. byes at `table_number = -1`/`0`). Page through to the true last result — a single page silently truncates. Dedupe by `table_number` (latest per table). |
| `next` type differs by endpoint | Unfiltered `matches-list` and `registrations-slim` return `next` as a **numeric page number** (e.g. `2`). The `status=in_progress` matches-list returns `next` as a **URL string**. Never pass `next` to `cardeGet()` when it could be numeric (`pathOrUrl.startsWith is not a function` at runtime) — use an explicit `page` counter with `&page=${page}` and treat `next` only as a boolean (`!== null`) for whether to continue. |
| `registrations-slim` exposes `fixed_seat` | `GET v2/organize/events/{id}/registrations-slim/?ordering=fixed_seat&page_size=200` → `fixed_seat: number | null` (assigned table; null = normal Swiss). `ordering=fixed_seat` sorts non-null first so you can early-stop. Join to matches on `user.id`. Powers the Fixed Seating Report. |
| `pairings_status` is the reliable "round has pairings" signal | `raw_carde_rounds.pairings_status = 'GENERATED'` is set as soon as pairings exist, **before** the timer starts. The normalized `rounds.carde_status` stays `UPCOMING` until the timer starts — do not use it to detect the current paired round. |

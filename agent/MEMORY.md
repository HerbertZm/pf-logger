...
### 2026-06-26 (session 3 — timer drift, play_started_at, coverage, judge field)
**Asked:** Fix timer showing earlier than Carde timer; populate `overtimeMinutes`; add "actual play start" as a DB column; fix judge field empty on log entries; capture coverage (seating) logs. Also asked who starts a round / who does bulk actions in PF.
**Learned:** `timerEndDatetime` was being overwritten on COMPLETE rounds with `startedAt + timerDurationMin`, losing the real value captured during IN_PROGRESS. Fixed to write-once for COMPLETE rounds. `play_started_at = timerEndDatetime − game.defaultRoundLengthMin` — **NOT** `timerDurationMin` (which is unreliable; seen as 16min on a 60min round). Worker fetches `game.defaultRoundLengthMin` and stores it as `play_started_at` in a new DB column (migration `20260619130000`). `overtimeMinutes` now computed as `lastMatchCompletedAt − timerEndDatetime`. `TableJudgeCall.judge` was never populated despite `coveredBy` being fetched — fixed, with backfill on next sync. Coverage logs (seating) were never captured because the PF fetch filtered on `judgeResult IS NOT NULL` — added separate `coveredBy IS NOT NULL` query and `normalizeCoverage()`. PF has no persistent audit log for round-start or bulk actions — `tournament_logs` is extensions only; `tournament_time` has no author; `table_status.updated_status_by` is current-round-only display name.

### 2026-06-26 (session 2 — fixed seating + insights)
**Asked:** Fixed seating report showed previous round after pairings generated (pre-timer window). Also shipped in this session: `opponentIsFixedSeat` flag + blue row highlight, sortable columns, section-scoped print fix, move fixed seating to top of Reports tab, seating gap column in Insights (removed copy button).
**Learned:** `raw_carde_rounds.pairings_status = 'GENERATED'` is the correct source for "does this round have pairings" — `rounds.carde_status` stays `UPCOMING` until timer starts, too late for the pairing-generated-but-timer-not-yet-started window. Fixed in `GET /api/fixed-seating` (commit `c8eb2d4`). Carde `next` pagination: `registrations-slim` and unfiltered `matches-list` return `next` as **numeric page number**; `status=in_progress` matches-list returns `next` as **URL string**. Never pass `next` to `cardeGet()` when it may be numeric — always use explicit page counter.

### 2026-06-26 (session 1 — round timing + prod outage)
...
### 2026-06-21
**Asked:** Build a printable fixed seating report: for the current round, show each fixed-seat player's name, assigned fixed seat, current table, and opponent — regardless of whether they were moved.
**Learned:** Carde `registrations-slim` endpoint (`GET /api/v2/organize/events/{id}/registrations-slim/?ordering=fixed_seat&page_size=200`) exposes `fixed_seat: number | null`; `ordering=fixed_seat` sorts non-null values first so pagination can early-stop. Join key to `matches-list` is `user.id` (integer, stable). `fetchCardeAllRoundMatches` fetches ALL matches per round (no `status` filter) at request time — worker DB only has `status=in_progress` matches. TS7022 error: naming a local variable `page` alongside a type with a `next` property in the same file triggers circular inference — rename variable to `matchPage` to resolve. `pathOrUrl.startsWith is not a function` at runtime: caused by passing numeric `next` value to `cardeGet()` — use page counter instead.

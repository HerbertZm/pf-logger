...
- Round timing report **play start** is `rounds.play_started_at` (set by worker when timer end is known; equals `timer_end_datetime − timer_duration_min`). Use this for play duration and seating turnover. Fall back to `started_at` only when `play_started_at` is null.

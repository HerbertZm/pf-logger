...
/** Safely convert an unknown cell value to a display string. Objects are JSON-stringified. */
...
const TableSection = ({ name, tournamentId }: TableSectionProps) => {
    ...
    const load = async (nextOffset: number) => {
        ...
        try {
            ...
            setData((prev) =>
                nextOffset === 0 ? result : { rows: [...(prev?.rows ?? []), ...result.rows], total: result.total },
            );
        ...
        <div className="data-tab__section">
            ...
            {open && (
                <div className="data-tab__section-body">
                    ...
                    {data?.rows.length === 0 && <p className="data-tab__empty">No rows.</p>}
...
export const DataTab = () => {
    ...
    return (
        <div className="data-tab">
            <p className="data-tab__note">Raw table explorer — read-only. Data reflects the last ingestion cycle.</p>
            {TABLES.map((name) => (
